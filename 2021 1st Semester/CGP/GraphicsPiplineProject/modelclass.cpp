...
bool ModelClass::Initialize(ID3D11Device* device, int Type)
{
	bool result;
	...
	if (Type == 1)
		...
		result = InitializeBuffersH(device);
	else
		result = InitializeBuffersP(device);
...
bool ModelClass::InitializeBuffersH(ID3D11Device* device)
{
	VertexType* vertices;
	unsigned long* indices;
	D3D11_BUFFER_DESC vertexBufferDesc, indexBufferDesc;
	...
	vertices[4].position = D3DXVECTOR3(0.0f, -2.0f, 0.0f);  // Bottom Middle.
	vertices[4].color = D3DXVECTOR4(1.0f, 0.0f, 0.0f, 1.0f);

	vertices[5].position = D3DXVECTOR3(0.0f, 2.0f, 0.0f);  // Top Middle.
...
bool ModelClass::InitializeBuffersP(ID3D11Device* device)
{
	VertexType* vertices;
	unsigned long* indices;
	D3D11_BUFFER_DESC vertexBufferDesc, indexBufferDesc;
